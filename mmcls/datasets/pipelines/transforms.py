...
class RandomChannelShuffle(object):
    """Shuffle the channels of image randomly.
    ...
    def __init__(self, prob=0.5):
        assert 0 <= prob <= 1
        self.prob = prob
        self.channel_idx = np.arange(3)

    def __call__(self, results):
        """Call function to shuffle channels of image.
        ...
        Returns:
            dict: Flipped results, 'flip', 'flip_direction' keys are added into
                result dict.
        """
        channel_shuffle = True if np.random.rand() < self.prob else False
        results['channel_shuffle'] = channel_shuffle
        if results['channel_shuffle']:
            np.random.shuffle(self.channel_idx)
            for key in results.get('img_fields', ['img']):
                results[key] = results[key][:, :, self.channel_idx]
        return results
...
class Normalize(object):
    ...
    def __init__(self, mean, std, to_rgb=True, is_3d_input=False):
    ...
    def __call__(self, results):
        if self.is_3d_input:
            ...
            mean = (self.mean[0], ) * d
            std = (self.std[0], ) * d
            results['img'] = mmcv.imnormalize(img, mean, std, self.to_rgb)
        else:
            for key in results.get('img_fields', ['img']):
...
class Albu(object):
    ...
    def __repr__(self):
        ...
        return repr_str
@PIPELINES.register_module()
class TensorNormCrop:
    def __init__(self, crop_size, move=5, train=True, mean=99., std=77., copy_channels=False):
...
class TensorNormCropRotateFlip:
    def __init__(self, crop_size, move=5, train=True, copy_channels=False, mean=99., std=77.):
        self.size = (crop_size, ) * 3 if isinstance(crop_size, int) else crop_size
        self.move = move
        self.copy_channels = copy_channels
        self.train = train
